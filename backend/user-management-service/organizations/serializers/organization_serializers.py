...
class OrganizationCreateSerializer(RestrictedFieldsMixin, serializers.ModelSerializer):
    ...
    def validate_type(self, value):
        ...
        if value not in valid_types:
            raise serializers.ValidationError(f"Invalid organization type. Must be one of: {', '.join(valid_types)}")
        return value
...
class OrganizationUpdateSerializer(RestrictedFieldsMixin, serializers.ModelSerializer):
    ...
    restricted_fields = {
        'id', 'name', 'type', 'is_active', 'legal_entity_identifier',
        'status', 'organization_admin', 'organization_admin_id',
    ...
    def validate_email(self, value):
        ...
        if self.instance and Organization.objects.filter(email=value).exclude(pk=self.instance.pk).exists():
            raise serializers.ValidationError("An organization with this email already exists")
        return value.lower()
    ...
    def validate(self, data):
        ...
        if not any(value not in (None, '') for value in data.values()):
            raise serializers.ValidationError("At least one non-empty field must be provided for update")
        return data


class OrganizationListSerializer(serializers.ModelSerializer):
    organization_admin_id = serializers.IntegerField(source='organization_admin.id', read_only=True)
    organization_admin_name = serializers.SerializerMethodField()

    class Meta:
        model = Organization
        fields = [
            'id', 'name', 'email', 'type', 'address',
            'legal_entity_identifier', 'status',
            'organization_admin_id', 'organization_admin_name'
        ]
...
class OrganizationDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'name', 'email', 'type', 'address',
            'legal_entity_identifier', 'status', 'is_active',
            'organization_admin', 'onchain_verifications',
            'created_at', 'updated_at'
        ]
        read_only_fields = fields
    ...
    def get_organization_admin(self, obj) -> Optional[dict]:
        if obj.organization_admin:
            return {
                ...
                'phone_number': str(obj.organization_admin.contact_number) if obj.organization_admin.contact_number else None,
...
class OrganizationResponseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'name', 'email', 'type', 'address',
            'legal_entity_identifier', 'organization_admin_id'
        ]
        read_only_fields = fields
