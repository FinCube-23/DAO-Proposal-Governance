...
class ProtectedAnalyticsController(ViewSet):    
    ...
    def get_analytics_data(self, request):
        ...
        """
        logger.set_context("get_analytics_data")
        logger.log({"message": "Getting analytics data Started", "data": request.query_params})
        
        try:
            analytics_data = AnalyticsService.get_all_stats()
            serializer = StatsResponseSerializer(data=analytics_data)

            logger.log({"message": "Getting analytics data Success", "data": serializer.initial_data})
            return Response(
            ...
            logger.error({"message": "Getting analytics data Error", "data": request.query_params, "error": str(e)})
            return Response(
