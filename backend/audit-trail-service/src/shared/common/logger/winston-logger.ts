...
export class WinstonLogger implements LoggerService {
        ...
        new winston.transports.Console({
          format: winston.format.combine(
            ...
            winston.format.printf(
              ({ level, message, timestamp, context, trace_id, span_id }) => {
  ...
  log(message: any, contextValue?: string) {
    this.logger.info(
      this.createLogEntry(message, 'info', undefined, contextValue),
    );
  }

  error(message: any, trace?: string, contextValue?: string) {
    this.logger.error(
      this.createLogEntry(message, 'error', trace, contextValue),
    );
  }

  warn(message: any, contextValue?: string) {
    this.logger.warn(
      this.createLogEntry(message, 'warn', undefined, contextValue),
    );
  }

  debug(message: any, contextValue?: string) {
    this.logger.debug(
      this.createLogEntry(message, 'debug', undefined, contextValue),
    );
  }

  verbose(message: any, contextValue?: string) {
    this.logger.verbose(
      this.createLogEntry(message, 'verbose', undefined, contextValue),
    );
  }
}
