import { CreatedProposalDto } from './dto/proposal-update.dto';
...
export class ProposalUpdateRepository {
  constructor(
    ...
    this.logger.setContext(ProposalUpdateRepository.name);
  }
  ...
  async getProposalsAdded(transactionHash: string): Promise<any> {
    try {
      if (!transactionHash) {
        this.logger.error(
          'Invalid transaction hash: Transaction hash is empty',
        );
        throw new Error('INVALID_TRANSACTION_HASH');
      }

      this.logger.log(
        `THE GRAPH: Fetching proposal added event data for transaction: ${transactionHash}`,
      );
      ...
      if (!proposals || proposals.length === 0) {
        this.logger.warn(
          `No proposals found for transaction hash: ${transactionHash}`,
        );
      ...
      if (
        ...
        this.logger.error(
          'Required proposal data missing',
          JSON.stringify({
      ...
      this.logger.log(
        'Proposal Added event found successfully',
        JSON.stringify({
          proposalId: proposal.proposalId,
          blockNumber: proposal.blockNumber,
          transactionHash: proposal.transactionHash,
      ...
      if (error.networkError) {
        this.logger.error(
          `Network error while fetching proposal for tx: ${transactionHash}`,
          error.networkError,
        );
      ...
      if (error.graphQLErrors?.length > 0) {
        this.logger.error(
          `GraphQL errors while fetching proposal for tx: ${transactionHash}`,
          error.graphQLErrors,
        );
      ...
      if (
        error.message.includes('NO_PROPOSAL_FOUND') ||
      ...
      this.logger.error(
        `Unexpected error while fetching proposal for tx: ${transactionHash}`,
        error,
      );
  ...
  async getTransactionsUpdated(transactionHashes: string[]): Promise<any> {
    try {
      ...
      transactionHashes = transactionHashes.filter((hash) =>
      ...
      this.logger.log(
        `Fetching event data for transactions: ${transactionHashes.join(', ')}`,
      );
      ...
      if (
        ...
        this.logger.warn(
          `No relevant events found for transaction hashes: ${transactionHashes.join(', ')}`,
        );
