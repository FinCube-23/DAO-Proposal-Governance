...
contract FinCubeDAO is Ownable {
 ...
 /**
     ...
     */
   enum ProposalType {
    NewMemberProposal, TokenAddressProposal, RoyaltyProposal
   }
...
*/
   function registerMember(
    ...
    function newMemberApprovalProposal(
        ...
        emit ProposalCreated(proposalCount, ProposalType.NewMemberProposal, _data);
   ...
   function newTokenAddressProposal(address _tokenAddress) external onlyMember(msg.sender) {
    ...
    emit ProposalCreated(proposalCount,ProposalType.TokenAddressProposal, _data);
    unchecked {proposalCount++;}
}
...
function castVote(uint256 _proposalId, bool _isYesVote) external onlyMember(msg.sender) {
        ...
        if (_isYesVote) {
           unchecked {proposal.yesvotes++;} 
            votes.isYesVote[msg.sender] = true;
        } else {
            unchecked{proposal.novotes++;}
    ...
    function executeProposal(uint256 proposalId) public {
       ...
       else if(proposalType[proposalId]==ProposalType.TokenAddressProposal){
